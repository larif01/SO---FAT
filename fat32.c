...
{
    return bpb_faddress(bpb) + bpb->n_fat * bpb->fat_size32; // Use fat_size32 for FAT32
}

/* calculate data address */
uint32_t bpb_fdata_addr(struct fat_bpb *bpb)
{
    return bpb_froot_addr(bpb) + (bpb->possible_rentries * 32); // Not used in FAT32, but kept for compatibility
}
